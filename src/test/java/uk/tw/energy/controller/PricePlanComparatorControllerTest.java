...
import uk.tw.energy.domain.Cost;
...
public class PricePlanComparatorControllerTest {
    ...
    @Test
    public void previousWeekCostNoDataAvailableShouldReturnErrorMessage() {
        assertThat(controller.previousWeekCost("not-found").getBody().getErrorMessage()).isEqualTo(controller.NO_COST_ERROR_MESSAGE);
    }
    @Test
    public void previousWeekCostDataAvailableShouldReturnValue() {
        // GIVEN
        Instant now = Instant.now();
        ElectricityReading electricityReading = new ElectricityReading(now.minus(7l, ChronoUnit.DAYS), BigDecimal.ONE);
        meterReadingService.storeReadings(SMART_METER_ID, Arrays.asList(electricityReading));

        // WHEN
        Cost cost = controller.previousWeekCost(SMART_METER_ID).getBody();

        // THEN
        assertThat(cost.getErrorMessage()).isEqualTo(BigDecimal.TEN);
        Assert.isNull(cost.getErrorMessage(), "Expected error message to be null!");
    }
